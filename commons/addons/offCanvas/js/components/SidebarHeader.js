...
export async function insertHeader() {
  if (options()?.headerFilter) {
    ...
    const headerTitle = `<h4 id="headerFilter" class="">
      <span i18n="sidebar.legend.title">Recherche avancée</span>
      <button onclick="resetFilter();" title="Réinitialiser">
        <svg
    ...
    sidebarHeader.insertAdjacentHTML("beforeend", headerTitle);
    // Init current Lang
